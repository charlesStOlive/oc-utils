 ...
 */
class RuleActionBase extends SubForm
{
    protected $morphName;

    public function __construct($host = null)
    {
        $this->morphName = 'actioneable';
        $this->init('/waka/utils/models/rules/fields_action.yaml');
        if (!$this->host = $host) {
            return;
        }
        $this->boot($host);
    ...
    public function transformClassToDotedView() {
        ...
        return [
            ...
            'viewPathCode' => $author.'.'.$plugin.'::wakaactions.'.$contentName,
            'viewPathUrl' => plugins_path($author.'/'.$plugin.'/views/wakaactions/'.$contentName),
            'themePathUrl' => themes_path('wakatailwind/partials/wakaactions/'.$contentName)
        ];
    }
    ...
    public function prepareDataForJs($type, $data) {
        return array_merge(
            [ "code" => $this->getCode() ],
            ['type' => $type],
            $data,
