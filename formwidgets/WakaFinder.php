 ...
 */
class WakaFinder extends FormWidgetBase
{
    ...
    /**
     * @var DataSource Relation column to display for the name
     */
    public $ds = 'data_source';

     /**
     * @var DataSource Relation column to display for the name
     */
    private $columnsConfig = null;

    /**
     * @var DataSource Relation column to display for the name
     */
    public $ds_instanciation = false;
     ...
     */
    public $title = 'backend::lang.wakafinder.find_record';
     ...
     * @inheritDoc
     */
    protected $defaultAlias = 'wakafinder';
    ...
    public function init()
    {
        ...
        if (!$this->useRelation && !class_exists($this->modelClass) && !$this->ds) {
            throw new ApplicationException(Lang::get('backend::lang.wakafinder.invalid_model_class', ['modelClass' => $this->modelClass]));
        }
        ...
        if($this->ds) {
            ...
            if(!$dsCode) {
                ...
                $ds = \DataSources::find($dsCode);
                $this->modelClass = $ds->class;
                $this->columnsConfig = "~/plugins/".$ds->modelPath."columns.yaml";
                $this->nameFrom = $ds->outputName;

                $this->ds_instanciation = true;
    ...
    public function onRefresh()
    {
        ...
        $this->prepareVars();
        return ['#'.$this->getId('container') => $this->makePartial('wakafinder')];
    }
    ...
    public function onFindRecord()
    {
        ...
        return $this->makePartial('wakafinder_form');
    }
    ...
    protected function makeListWidget()
    {
        ...
        $config->recordOnClick = sprintf("$('#%s').wakaFinder('updateRecord', this, ':" . $this->keyFrom . "')", $this->getId());
    ...
    protected function makeSearchWidget()
    {
        ...
        $widget->cssClasses[] = 'wakafinder-search';
        return $widget;
    }
}
