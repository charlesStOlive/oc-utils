...
class CreateModelControllerV2 extends GeneratorCommand
{
    ...
    protected function prepareVars()
    {
        ...
        $relations = $relations->map(function ($item, $key) {
            $relationData = $rows->where('var', $item['var'])->first()->toArray();
            $item = $this->getRelations($item, $relationData) ;
            return $item;
        });
        trace_log($relations);

        $this->config['belong'] = $relations->where('type', '==', 'belong')->toArray();
        $this->config['oneThrough'] = $relations->where('type', '==', 'oneThrough')->toArray();
        $this->config['belongsMany'] = $relations->where('type', '==', 'belongsMany')->toArray();
        $this->config['many'] = $relations->where('type', '==', 'many')->toArray();
        $this->config['manyThrough'] = $relations->where('type', '==', 'manyThrough')->toArray();
        $this->config['morphMany'] = $relations->where('type', '==', 'morphMany')->toArray();
        $this->config['morphOne'] = $relations->where('type', '==', 'morphOne')->unique('relation_name')->toArray();
        $this->config['attachOne'] = $relations->where('type', '==', 'attachOne')->toArray();
        $this->config['attachMany'] = $relations->where('type', '==', 'attachMany')->toArray();



        //Trsnfomation des listes uniques. 
        $this->config['lists'] = $relations->where('lists', '!=', null)->unique('lists')->toArray();
    ...
    public function getRelations($item, $relationData)
    {
        ...
        $toolbar =  $item['toolbar'] ?? false;
        $recordUrl =  $item['recordUrl'] ?? false;
        //
        $fieldType = $relationData['field_type'];
        //
        $relationClass = $this->getRelationClass($dotedClass, $var);
        $relationPath = $this->getRelationPath($dotedClass, $var, $createYamlRelation);
        ...
        if ($type == 'belong') {
            ...
            $item['belong'] = [
                ...
                'createYamlRelation' => $createYamls,
        ...
        if ($type == 'morphOne') {
            ...
            $createRelationInController = $fieldType == 'partial_relation';
            $item['morphOne'] = [
                'relation_name' => $this->getRelationKeyVar($array[1], $var),
                'relation_class' => $relationClass,
                'singular_name' => str_singular(camel_case($var)),
                'relation_path' => $relationPath,
                'options' => $options,
